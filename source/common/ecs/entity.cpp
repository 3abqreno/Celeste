...
namespace our
{
    ...
    glm::mat4 Entity::getLocalToWorldMatrix() const
    {
        ...
        glm::mat4 localToWorld = localTransform.toMat4(); // Get the local transformation matrix

        const Entity *currentEntity = this; // Start with the current entity

        // Traverse up the hierarchy until we reach the root entity
        while (currentEntity->parent != nullptr)
        {
            // Multiply the current localToWorld matrix with the parent's localToWorld matrix
            localToWorld = currentEntity->parent->localTransform.toMat4() * localToWorld;
